...
remove_version_from_arxiv_ids = importlib.import_module('arxiv_vanity.papers.migrations.0014_remove_version_from_arxiv_ids').remove_version_from_arxiv_ids


class RemoveVersionsFromArxivIDsTest(TestCase):
    def test_remove_version_from_arxiv_ids(self):
        paper = create_paper(arxiv_id='1708.03311v1', arxiv_version=0, arxiv_url='https://arxiv.org/abs/1708.03311v1', pdf_url='https://arxiv.org/pdf/1708.03311v1')
        ...
        self.assertTrue(Paper.objects.deleted().filter(arxiv_id='1708.03313v2').exists())

        # ensure paper without version already existing is handled correctly
        self.assertEqual(Paper.objects.get(arxiv_id='1708.03314').pk, paper_without_render.pk)
        self.assertTrue(Paper.objects.deleted().filter(arxiv_id='1708.03314v2').exists())
